...
unsigned long           encoderSpeedValue = tickInterval;     //start value to speed -> tick ->
unsigned long           encoderSpeedCenter = tickInterval;    //start value to speed -> tick ->
#define                 encoderSpeedMinValue  150            //min value in ms for speed encoder 100ms
#define                 encoderSpeedMaxValue  1400           //max value in ms for speed encoder 1400ms
#define                 encoderSpeedStepValue 5            //step value for speed encoder
...
Encoder encoderShift(8, 9);       //shift interface encoder
Encoder encoderSpeed(22, 23);     //speed interface encoder

Encoder encoderK(32, 33);         //k euclidian parameter interface encoder
Encoder encoderN(34, 35);         //n euclidian parameter interface encoder
...
void tickInterrupt() {
  if (externalClock) {
    ...
    tickInterval = (tickInterval < encoderSpeedMinValue) ? encoderSpeedMinValue : tickInterval;   //minimum value of 100
    tickLast = millis();
    firetriggers();
...
void loop()
{
  ...
  if (thisLoop > interfaceEvent + interfaceDebounce) {
    ...
    if ( (thisLoop > (tickLast + triggerTime + 1))  &&
         ...
         (thisLoop < (tickLast + tickInterval - 20) ) ) { //if NOW is after triggers off and NOW if before the start of next pulse
...
//this function should be improved changing the setLed to a faster setColumn
void refreshLeds() {
